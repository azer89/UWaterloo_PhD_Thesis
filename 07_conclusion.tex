...
\nnewtext{We would like to create element arrangements that have varying negative space widths.
...
Conversely, high density area can be represented by bigger elements and narrower negative space. 
...
As the arrangement is spacious, we can run RepulsionPak simulation without the growth process.
...
Additionally, generating a pleasing arrangement from a fully automated method is difficult, 
so it also would be interesting to incorporate an interactive tool by Reinert et al.~\cite{Reinert2013} 
that can infer an artist's intention to create an artistic layout.
}
