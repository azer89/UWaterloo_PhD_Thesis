...
\nnewtext{We would like to create element arrangements with varying and wider negative space gaps.
...
Additionally, we can adapt AnimationPak to generate animated discrete texture, in a similar style to work by Ma et al.~\cite{Ma2013}. 
...
\nnewtext{To realize this idea, it should be possible to adapt RepulsionPak by computing geodesic distances to generate physical forces.}
...
\nnewtext
{
It would also be interesting to create a system to assist an artist for creating arrangements of physical elements
by using a camera-equipped augmented reality headset. We use the camera to scan these elements which are then processed by the system.
The headset displays a suggestion of an element arrangement which the artist can follow.
As the artist moves the elements around, the system rescan the current element placement and recompute a new suggestion.
}
