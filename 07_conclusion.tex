...
\nnewtext{We would like to create element packings with varying widths of negative space channels.
...
In a low density area, elements are smaller and negative space is wider. On the other hand, high density area can be simulate with bigger elements and narrower negative space. 
It would also be great to use a bitmap image as a container, we then perform image segmentation algorithm to produce subregions, each has a different element density.
Additionally, generating a pleasing result from a fully automated method is difficult, so it also would be interesting to incorporate an interactive tool by Reinert et al.~\cite{Reinert2013}.
}
...
In discrete textures, the element distribution is more important than element interlocking.
As the arrangement is not tight, we can run RepulsionPak simulation without the growth process.
...
\nnewtext{We can also create elements from salient regions in a source photograph which
can be detected using an image segmentation algorithm~\cite{Comaniciu2002, Achanta2012} and a salient map.
Once these regions are obtained, each can be vectorized to create a new fixed element.
Another idea is to use a data-driven method to find a compatible element, 
which later can be significantly deformed to fit a salient region.
}
%Another idea is to identify salient shapes in a source photograph and
