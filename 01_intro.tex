...
\nnewtext{The subset of the container that does not belong to any element is
...
\item \textbf{Flow.} In local parts of a composition, the elements
  ...
  each is oriented toward the center of the container.}   
...
and we only require a small library of element shapes.}
\nnewtext{
...
It would be nice to say *something* in the introduction about physics.  Not necessarily here, but perhaps earlier.  A lot of the techniques developed in the thesis are motivated by physical simulation, even though the simulation itself is not the final goal. In this work, simulation of physical forces can be thought of as a kind of optimization process, with the resulting physical equilibrium as the desired output.
