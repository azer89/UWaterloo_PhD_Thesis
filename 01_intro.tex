...
to the construction of packings. A few of them are adapted from work by Wong et al.~\cite{Wong1998}.
\nnewtext{
These principles are design considerations that cover a broad range of packing styles. 
A packing algorithm need not satisfy all five of them.
...
\item \textbf{Balance.} A composition does not exhibit too
  ...
  \newtext{The primary elements \newtext{in} Figure~\ref{fig_primary_secondary} come in a range of sizes, but none so extreme that they stand out. 
...
\item \textbf{Uniformity Amidst Variety.} Repeated elements must balance
  ...
  Gombrich \newtext{also wrote} eloquently on the role of variation in 
  design~\cite{Gombrich}.
  \newtext{In the context of procedural art, Galanter discussed the trade off between order and disorder~\cite{Galanter2003}.}
...
We define the general packing problem to be solved as follows. 
\nnewtext{
The user provides several general inputs, although an individual packing method requires additional information from time to time.}
We discuss these inputs below:

\begin{enumerate}
      \item \newtext{A library of elements. Each is a collection of open, closed, or intersecting polygonal paths.
            An element can contain additional information, such as \nnewtext{colours}, a preferred orientation, or a scripted animation.}
      \item \newtext{One or more target containers. Each container is a closed non-self-intersecting curve to be filled with elements.
            A container can optionally have internal holes.}
      \item \newtext{An optional set of fixed elements that we transfer directly to the result.}
\end{enumerate}

\nnewtext{Our goal is to fill each container with elements, trying to satisfy several considerations:}

\begin{enumerate}
  \item \nnewtext{Balance out the element distribution and make the widths of negative space be as uniform as possible.}
  \item \newtext{Deform elements in a controllable way, trading off between the preservation of element shapes and the creation of shape compatibilities.}
  ...
  \item \newtext{Use the container space efficiently by having as little negative space as possible.}
  \item \newtext{Conform to container boundaries.}
\end{enumerate}
...
and \nnewtext{present} \newtext{a study to investigate} the quantitative evaluation of packing quality in greater detail:
\begin{enumerate}
...
\item RepulsionPak is a packing method that utilizes repulsion forces to distribute and deform elements,
	\newtext{each represented} as a mass-spring system (Chapter~\ref{chapter_repulsionpak}).
