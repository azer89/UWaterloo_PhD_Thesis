...
%(Figure~\ref{fig_logo_packing}, \ref{fig_graphic_designs}, \ref{fig_woodland}).
%\mynote{Explain each figures}.
...
%In Figure~\ref{fig_graphic_designs}X the artist artfully placed small triangle or circle elements
%to eliminate remaining large gaps of negative space without improving element interlocking.

%\mynote{need to juggle sentence in paragraph 2}
%%An artist distributes elements so that they communicate the shape of the container. 
%Creating a packing is a challenging process.
%%The artist should align the elements with the container boundary to emphasize the container shape.
%The artist should align elements in a way that their boundaries interlock with each other,
%causing the separation between neighbouring elements to become roughly the same everywhere.
%%Unlike jigsaw puzzles that have perfect interlocking, 
%%packings contain unused container space that does not belong to any element,
%%which is called \textit{negative space} (Figure~\ref{fig_logo_packing}b).
%The unused space that does not belong to any element is called \textit{negative space} (Figure~\ref{fig_logo_packing}b).
...
%In Figure~\ref{fig_graphic_designs}X the artist artfully placed small triangle or circle elements
%to eliminate remaining negative space without improving element interlocking.

Elements can also be oriented to change the packing aesthetics.
...
We are also interested in methods for measuring the evenness of negative space
