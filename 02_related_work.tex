...
\nnewtext
{
Past research has proposed methods~\cite{Kaplan2004, Sugihara2009, Lin2018} 
to replicate the figure-ground reversal style of Escher's Sky and Water,
in which elements serve as both positive and negative space depending on the viewer's perception.
...
creating a ``fading effect'', $S_1$ and $S_2$ disappear to become negative space.
...
A framework to generate the metamorphosis tiling is proposed by Kaplan~\cite{Kaplan2010}.
}
...
\caption[Two 3D packings and a packing on a surface]
{\label{fig_related_gal_chen} 
\nnewtext
{
(a) An Arcimboldo-like packing of overlapping 3D elements~\cite{Gal2007B}. 
(c) A packing of non-overlapping 3D elements.
(c) Elements that fill a surface which are connected by hinges~\cite{Chen2017}.
}
}
\end{figure}
