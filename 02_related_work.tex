...
\newtext
{
Secord~\cite{Secord2002} distributed point elements using a weighted centroidal Voronoi diagram.
Given an input greyscale image, pixel intensities are incorporated into the centroid calculation, 
so that darker regions have more point element densities and the resulting point distribution
resembles a stippling drawing.
}
...
An element is represented as a single point, which is adequate for small rigid elements.
