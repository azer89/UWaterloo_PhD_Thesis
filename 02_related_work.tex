...
\nnewtext{The computation of Voronoi diagrams can be generalized to any shapes for which a distance measurement exists.} 
\newtext
...
\nnewtext
{
An alternative approach to generate packings is to use data-driven methods that
requires an input element library (Figure~\ref{fig_related_jim_pad}).}
\newtext{
This is usually done by placing elements one by one until the container area is filled.
...
Pyramid of Arclength Descriptor (PAD)~\cite{Kwan2016} is a shape descriptor that
...
(Left) Vertumnus painting by Giuseppe Arcimboldo (Skokloster Castle, Skokloster, Sweden) that illustrates
a 3D packing of non-overlapping plants, vegetables, and fruits. 
(Right) An Arcimboldo-inspired 2D packing~\cite{Huang2011} consisting of overlapping elements.
}
}
\end{figure}
