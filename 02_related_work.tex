...
Figure~\ref{fig_taxonomy} shows the taxonomy of elements arrangement subcategories.
The taxonomy is more focused on packings because our proposed methods do not attempt to generate tilings or discrete textures.
Many related methods discussed in this chapter aim to minimize element overlaps. 
Looking closely, a few packings shown in this chapter 
contain overlaps to some degree due to the challenge of 
finding and aligning compatible elements.
}
...
\nnewtext
{
This section discusses previous methods to arrange 2D elements in the plane.
...
An approach to generate a packing is to start with an initial configuration and iteratively refine it using \textbf{Lloyd's method}
to obtain \nnewtext{an even distribution of elements, which means the distribution of positive and negative space is uniform.}
...
a variant of CVD that is a distribution of polygonal elements.
...
\newtext{\textbf{Data-Driven Methods:} 
...
\newtext{It is also possible to initially partition the container into smaller segments
