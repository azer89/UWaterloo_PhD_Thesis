...
\nnewtext
{
An informal classification of element arrangements.
}
}
\end{figure}
...
Based on their designs, Figure~\ref{fig_taxonomy} elements arrangements can be categorized into 
packings, tilings, and discrete textures.
The categorization can be further expanded but we only choose ones that are related to our proposed packing methods.
...
Although many packing methods discussed in this chapter aim to minimize element overlaps, 
...
\nnewtext
{
In general, many mosaicing and packing methods can be grouped into three categories (Figure~\ref{fig_taxonomy_method}).
The first category, Lloyd's method, iteratively refine an initial element arrangement to generate a more even distribution.
Lloyd's method is popular in many mosaicing techniques that attempt to pack convex or near convex elements.
The second category, data-driven methods, place elements one by one. For each step, a data-driven method select 
an element from an element library that is compatible with previously placed elements and the container boundary.
The last category, deformation-driven methods, attempt to create element compatibilities through shape deformation.
Data-driven methods and deformation-driven methods are commonly used in packing methods with concave elements.
}
...
Early mosaicing techniques used Lloyd's method to pack simple convex elements
inside containers. Later techniques generalized Lloyd's method to pack
more complicated concave elements.
}

\nnewtext
{
We first discuss the simplest variant of Lloyd's method that distributes point elements~\cite{McCool1992}, as shown in Figure~\ref{fig_lloyds_method}.}
\newtext{Given $N$ point elements, 
...
\nnewtext{A standard CVD assumes the area density is uniform everywhere.}
...
\nnewtext{
His method incorporates the pixel intensities of an input image as area densities, 
which alter the centroid calculation so that point elements are attracted to low-intensity regions.}
...
\nnewtext
{
(a) Stippling artwork generated using Lloyd's method that incorporates pixel densities of the input image~\cite{Secord2002}.
(b) Mosaics of square elements generated using Lloyd's method with Manhattan distance~\cite{Hausner2001}
}
}
\end{figure}

\nnewtext{The computation of Voronoi diagrams can be generalized 
...
In follow up work, Smith et al.~\cite{Smith2005} generated temporally coherent animated mosaics by utilizing CAVDs.
...
earlier methods. However, the FFT-based method cannot rotate the elements,
so they resorted to a brute force approach to find the best orientation.}
...
their boundaries are distorted to eliminate overlap, to create shape variations, 
...
\nnewtext
{
This section discusses packing methods that fall into the category of data-driven and deformation-driven.
Unlike mosaics, these packings have different visual styles as they are composed of elements with concave shapes,
such as such as man-made objects, plants, or animals.
Additionally, the visual styles of packings are inspired by Giuseppe Arcimboldo's paintings 
(Figure ~\ref{fig_related_arcimboldo_optimus}a).
}

\nnewtext
{
A data-driven method require an input element library (Figure~\ref{fig_related_jim_pad}).}
\newtext{and place elements one by one until the container area is filled.
...
\newtext{It is also possible to initially partition a container into smaller segments
...
The more elements in the library, the better the chances of finding compatible element boundary.}
\nnewtext
{
For example, JIM method required a library with 900 elements, PAD used a library with more than 100 elements, 
...
\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{figures/related/arcimboldo_optimus.pdf} 
\caption[Examples of collages]
{\label{fig_related_arcimboldo_optimus} 
\nnewtext
{
Examples of element collages.
(a) Vertumnus painting by Giuseppe Arcimboldo (Skokloster Castle, Skokloster, Sweden) that illustrates
a 3D packing of non-overlapping plants, vegetables, and fruits. 
(b) An Arcimboldo-inspired 2D packing~\cite{Huang2011} consisting of overlapping elements.
}
}
\end{figure}
...
\nnewtext{ 
Both methods permit some elements to overlap.  
While they can both correct gaps and overlaps using deformation, the deformation
