...
{
\nnewtext
{Many packing algorithms can also be grouped according to their main algorithmic technique} (Figure~\ref{fig_taxonomy_method}).
...
Originally, Lloyd's method \nnewtext{was introduced in computer graphics} by McCool and Fiume to distribute point elements~\cite{McCool1992}.
It was is generalized to distribute convex and concave elements.
}
...
\nnewtext{His method incorporates the pixel intensities of an input image to construct a density function. 
This alters the centroid calculation so that point elements are attracted to darker regions.}
...
earlier methods.}
\newtext{\nnewtext{Similar previous variants,} the spectral approach cannot rotate the elements,
...
a sketch-based interface where an artist can draw curves \nnewtext{to guide the placement of square-like elements.}
...
\newtext
{
As elements reach perfect compatibility, a packing turns to a tiling, as seen from an example in Figure~\ref{fig_related_escherization}a.
...
\nnewtext{Given an input element $S$, they performed a search in a parameterized tiling space to find its
deformed variant $T$.
Due to deformation,}
...
\nnewtext{Two elements $S_1$ and $S_2$ are placed at opposite ends of a canvas.
...
computing contact point networks of rigid elements, \nnewtext{which can be seen in Figure~\ref{fig_related_gal_ma_chen}d}.
