...
AlMeraj et al.~\cite{AlMerajEval2013} conducted a qualitative evaluation to find
...
These user studies concerned about element distributions, but not element interlocking or the evenness of negative space, 
...
to each of three categories: their subjective preference, element interlocking, and stylishness.
}
...
but we get inspiration from their work to develop a metric based on shape overlaps that is discussed in the next section.
...
GLCM works best for analyzing dense point distributions and it unfortunately cannot be used to analyze geometric element arrangements.
The packing quality in fabrication~\cite{Chen2016, Zehnder2016, Martinez2019} 
is dictated by manufacturing constraints, such as connectivity,
so that printed objects can resist from breaking.
However, our objective is more about the evenness of negative space and less about strengths.
}
...
We investigated the idea of analyzing $r$ values only on a medial axis since they can be considered as the widths of negative space gaps.
