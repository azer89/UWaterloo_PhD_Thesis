...
\nnewtext
{
	...
	disc of radius $r$, chosen uniformly at random within the container region (drawn as the gray region), lies entirely within the 
	packing's negative space (drawn as red region).
	Geometrically, we compute the value of SCP as the ratio between the inward-offset negative space area with the container area.
...
(b) The distance transform of the negative space, the brighter the color of a pixel, the farther it is from the nearest boundary.
...
and the strengths of negative space springs are unchanged.}
...
First, we would like to conduct experiments that investigate the
...
Second, we would like to investigate a better visualization for SCP.
...
Third, it would be worthwhile to find a pseudodistance as a basis for our metrics
