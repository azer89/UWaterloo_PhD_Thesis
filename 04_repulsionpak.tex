...
\newtext{Repulsion forces} between neighbouring meshes work to even out the negative
...
we use \newtext{spring forces} within a mesh to preserve element shapes.}
...
A packing example with primary and secondary elements \newtext{was} shown in Figure~\ref{fig_primary_secondary}.
}
...
\newtext{Elements in FLOWPAK could only bend along their spines. 
In RepulsionPak, we allow elements to deform in a more open-ended way.}
RepulsionPak is intended to fulfill three of five design principles:
balance, uniformity amidst variety, and boundaries \newtext{(Chapter~\ref{chapter_introduction}).} 
...
\newtext{Lloyd's} method, data-driven methods, and deformation-driven methods.
...
Forces do not act directly \newtext{on} elements, but their mesh vertices.
During the simulation, these mesh vertices are spread apart but \newtext{remain} connected using elastic springs,
effectively creating an element representation that can be packed and deformed.
Additionally, unlike previous variants of \newtext{Lloyd's} method, 
repulsion forces \newtext{naturally induce rotations in elements, helping them discover greater compatibilities with their neighbours.} 
}
...
Generalizations of Lloyd's method have been proposed by
...
labyrinthine paths \newtext{using a geometric analogue of reaction-diffusion}. 
...
different \newtext{from that} of RepulsionPak (2D graphic design) and our results appear qualitatively
different.
}
...
RepulsionPak \newtext{uses} a similar approach, but we represent an element
...
\nnewtext{
RepulsionPak requires several pieces of input, which are described in Section~\ref{problem_formulation}.
Elements are then categorized as primary and optional secondary elements, such as those shown in Figure~\ref{fig_repulsionpak_pipeline}a.
Additionally, an artist needs to specify the desired element spacing distance $d_\mathrm{gap}>0$.
}
...
\newtext{The containers are \newtext{seeded} with copies of scaled-down elements.
This initial element placement can be done in two ways: random seeding (Section~\ref{repulsionpak_preprocessing}) 
or a careful placement using a shape matching algorithm (Section~\ref{repulsionpak_shape_matching}).}

%\nnewtext{The containers are then seeded with randomly placed
...
simulation. \newtext{The computation of barycentric coordinates is discussed in the next section.}
...
\newtext{Given an input element (Figure~\ref{fig_elements_image}a)}, we generate a skin, 
which is a simple closed polygon that fully encloses it \newtext{(Figure~\ref{fig_elements_image}b)}.
...
to obtain a simpler polygon $s'$, \newtext{which is the outer boundary of the mesh (Figure~\ref{fig_elements_image}c)}.
We then construct a Delaunay triangulation of $s'$ \newtext{(Figure~\ref{fig_elements_image}d)}.
...
\newtext{
We also add extra edges \nnewtext{to reduce}
folding and self-intersections during simulation.  First, if two triangles 
$ABC$ and $BCD$ share an edge $BC$, then we add a \textit{shear edge}
connecting $A$ and $D$ (Figure~\ref{fig_elements_image}e). 
...
created triangulation edges \newtext{(Figure~\ref{fig_elements_image}f)}. 
These negative space edges are used exclusively for internal
bracing.  The element's concavities can still be occupied by its neighbours.
...
\newtext{as illustrated in Figure~\ref{fig_repulsionpak_pipeline}}.
\begin{enumerate}
...
\newtext{deformed mesh vertices} connected by a spring.  We compute the spring force as follows:
\begin{equation}
\label{eq_edge_force}
...
\begin{packeddescriptions}
	...
	\item[$\ell$] $= \|\bm{x_{b}} - \bm{x_{a}}\|$ \newtext{which is the rest length of the spring}; and
...
This change allows the meshes \newtext{to be more tolerant of small displacements} and 
...
progresses.  
\newtext{
Every element mesh $M$ maintains a current scaling factor $g$.
When a mesh is permitted to grow, we increase its $g$ value by $k_g \Delta t_\mathrm{sim}$,
where $k_g$ is the growth rate and $\Delta t_\mathrm{sim}$ is the simulation time step.
We then compute the new rest lengths of all springs by scaling the current rest lengths by a factor of $g$.
These new rest lengths are then used as the $\ell$ values in Equation~\ref{eq_edge_force}
to calculate edge forces.
In our implementation, The constant $k_g$ is set to 0.01.
}


%Every element mesh $M$ has a counter $n_M$ that records the number of
%times it has grown.  When a mesh is permitted
%to grow, we add 1 to the counter.  Then, if $\ell_i$ is the rest length
%of the $i$th spring in the original undeformed mesh, we increase its rest
%length to \newtext{$(1+ n_M k_g \Delta t_\mathrm{sim})\ell_i$}.  The constant $k_g$,
...
\newtext{For example, the Unilever logo in Figure~\ref{fig_logo_packing} has a positive space ratio of $45\%$.
Additionally, }concave elements with long extensions are more difficult to pack, 
so a lower positive space ratio is recommended to generate a satisfactory packing.
%For example, the Unilever packing in Figure~\ref{fig_logo_packing} has positive space ratio of $45.025\%$.}
...
approach taken by human artists by filling these pockets \nnewtext{with} small, usually
...
\newtext{as seen in the rhinoceros's front horn in Figure~\ref{rhino_bad}.}
...
\nnewtext{$0.6A_c/n_\mathrm{e}$}, where $A_c$ is the container area and \nnewtext{$n_\mathrm{e}$} is the 
...
\end{equation}}
\nnewtext{In our implementation, we estimated curvature from the discrete sample points
using second-order forward finite differences~\cite{Banchoff2015}, although
we recently found that second-order central finite differences can give more accurate approximations.
Lastly, we} compute $\tau$ by summing curvature estimates using the trapezoid rule.
...
\caption[Global and local shape matching]{
  ...
  The element in (a) is completely inside, but the element in (b) has \newtext{non-local regions} that
...
\newtext{An example of this problem is illustrated in Figure~\ref{fig_global_local}.}
...
for points farther inside the container.  
\nnewtext{In our implementation we choose $\alpha = 1$ and $\beta = 0.001$.}
Sample points outside the container
...
\nnewtext{where} 
\begin{packeddescriptions}
\item[$n_\mathrm{p}$] \nnewtext{is the number of sample points on the element.}
\end{packeddescriptions}
...
\newtext{We let real-world artists and users to use RepulsionPak
for generating packings in Figure~\ref{paul_daichi_packings} and~\ref{adobe_packing},
but we unfortunately were unable to collect their statistics.}
%We were unable to collect statistics for packings in }
%\nnewtext{Two artists used RepulsionPak to generate packings in Figure~\ref{paul_daichi_packings},
%although we were unable to collect the statistics of those packings.}
...
\newtext{The supplemental materials include movies that visualize the simulation process}.  
...
\cellcolor{lg}\makecell{Primary\\elements}  & 
\cellcolor{lg}\makecell{Secondary\\elements} & 
\cellcolor{lg}Vertices   & 
\cellcolor{lg}\makecell{Primary\\running\\time (seconds)} & 
\cellcolor{lg}\makecell{Secondary\\running\\time (seconds)} & 
\cellcolor{lg}Iteration\\ \hline


\makecell{Animals\\(Figure~\ref{fig_repulsionpak_pipeline})}
& 25    & 14   & 2412  & 133  & 65  & 16670\\ \hline

\makecell{Rhino\\(Figure~\ref{rhino_packing})}
& 107   & 0    & 4833  & 237  & 0   & 15521\\ \hline

\makecell{Cats\\(Figure~\ref{cat_packing})}
& 41    & 69   & 3598  & 185  & 62  & 8531\\ \hline

\makecell{Birds\\(Figure~\ref{three_packings}a)}
& 43    & 43   & 2309  & 102  & 54  & 11571\\ \hline

\makecell{Bats\\(Figure~\ref{three_packings}b)}
& 47    & 22   & 3048  & 165  & 56  & 13120\\ \hline

\makecell{Butterflies\\(Figure~\ref{three_packings}c)}
& 123   & 135  & 11916 & 696  & 616 & 14379\\ \hline
...
% Butterflies (Figure~\ref{three_packings}c)
%Giraffes \& Penguins \\(Figure~\ref{giraffe_penguin_packing}) 
% Paisley (Figure~\ref{paisley_packing})
%Circles in Paisleys (Figure~\ref{paisley_packing}) 
...
\newtext{\newtext{Two professional artists} have used RepulsionPak to create packings shown in Figure~\ref{paul_daichi_packings}.
The skull packing is made of butterfly and plant elements arranged inside multiple containers.
The skull's teeth \newtext{are} made of manually placed leaf elements. 
The background is created by superimposing several packings.
The smoothies packing is made of fruit elements arranged inside a smoothie cup.
\newtext{Additionally, a casual user made a packing using RepulsionPak, which was then printed on
t-shirts for a fitness club at a tech company, as shown in Figure~\ref{adobe_packing}.}}


\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{figures/repulsionpak/paul_daichi.pdf} 
\vspace{-10pt}
\caption[Artist-made Packings using RepulsionPak]
{\label{paul_daichi_packings}
\newtext{
Artist-made packings using RepulsionPak. 
The top arrangement was made by Paul Trani, consisting of multiple containers filled with butterfly and plant elements.
The manually-placed leaves are fixed elements that represent the skull's teeth.
The background was made by superimposing several packings.
The bottom packing was made by Daichi Ito, 
where fruit elements are arranged inside a smoothie cup.
}
}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=1.0\textwidth]{figures/repulsionpak/adobe_packing.pdf} 
\caption[A RepulsionPak packing printed on a t-shirt]
{\label{adobe_packing}
\newtext{
A RepulsionPak packing printed on a t-shirt, created by Paul Asente.
}
}
\end{figure}


%The top arrangement was made by Paul Trani, where elements are packed inside several containers.
%The top arrangement was made by Paul Trani, consisting of multiple containers filled with butterlfy and plant elements.

%The manually-placed leaves serve as fixed elements that represent the skull's teeth.
%For the skull's teeth, the leaves are placed manually. 
...
\newtext{
\newtext{We see several opportunities for} further improvements to RepulsionPak.
We deliberately chose a simple simulation model based on springs and forward Euler integration,
  ...
  \newtext{future work should} experiment with several to investigate if any offers
  a suitable trade-off between performance and quality.
  Additionally, our barycentric warping method can
  ...
  work of Jacobson et al.~\cite{Jacobson2011}, Liu et al.~\cite{Liu2014}, and James~\cite{James2020}}.}