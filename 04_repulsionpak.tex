...
RepulsionPak requires several \newtext{inputs}, which are described in Section~\ref{problem_formulation}.
...
\newtext{$0.6A_c/n_\mathrm{e}$}, where $A_c$ is the container area and \newtext{$n_\mathrm{e}$} is the 
...
\end{equation}}
\newtext{In our implementation, we estimated curvature from the discrete sample points
...
\newtext{In our implementation we choose $\alpha = 1$ and $\beta = 0.001$.}
...
\newtext{where} 
\begin{packeddescriptions}
\item[$n_\mathrm{p}$] \newtext{is the number of sample points on the element.}
\end{packeddescriptions}
