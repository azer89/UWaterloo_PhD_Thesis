...
\nnewtext{Repulsion forces} between neighboring meshes work to even out the negative
...
we use \nnewtext{spring forces} within a mesh to preserve element shapes.}
...
\nnewtext{The containers are then seeded with copies of scaled-down elements.
...
simulation. \nnewtext{The computation of barycentric coordinates is discussed in the next section.}
...
\caption[Element discretization]{
	...
	\nnewtext{(b) A triangle mesh (black) with additional shear edges (red) and negative-space edges (dashed blue).}
}
\end{figure}
...
(Figure~\ref{fig_elements_image}b).  First, if two triangles 
...
\nnewtext{as illustrated in Figure~\ref{fig_repulsionpak_pipeline}}.
\begin{enumerate}
...
\nnewtext{deformed mesh vertices} connected by a spring.  We compute the spring force as follows:
\begin{equation}
\label{eq_edge_force}
...
\begin{packeddescriptions}
	...
	\item[$\ell$] $= \|\bm{x_{b}} - \bm{x_{a}}\|$ \nnewtext{which is the rest length of the spring}; and
...
\nnewtext{we find that most of our results have positive space ratios between $35\%$ and $60\%$ depending on the element shapes.
...
\nnewtext{as seen in the rhinoceros's front horn in Figure~\ref{rhino_packing}.}
...
(c) The final result.
\nnewtext{(d) Another result without the use of shape matching, 
a few elements are not aligned properly with convex corners: a long thin element is severely deformed in the front horn, 
and two elements cannot emphasize the shape of the back leg properly.}
}
}
\end{figure}
...
\nnewtext{An example of this problem is illustrated in Figure~\ref{fig_global_local}.}
...
\nnewtext{The supplemental materials include movies that visualize the simulation
process}.  
...
{
\nnewtext{
   Data and statistics for the RepulsionPak results.  
   ...
   We were not able to collect statistics from artist-made RepulsionPak packings in Figure~\ref{paul_daichi_packings}}
   }
...
\nnewtext{Real-world artists have used RepulsionPak to create packings shown in Figure~\ref{paul_daichi_packings}.
The skull packing is made of butterfly and plant elements arranged inside multiple containers.
The skull's teeth is made of manually placed leaf elements. 
...
Additionally, RepulsionPak was also used to create a packing printed on
...
\nnewtext{
As nothing is perfect, we see further improvements to RepulsionPak.
