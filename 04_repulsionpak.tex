...
Forces do not act onto elements, but their vertices.
During the simulation, these vertices are spread apart but still connected using elastic springs,
allowing RepulsionPak to pack and deform the elements.
%RepulsionPak can be categorized as a variant of Lloyd's method without explicitly computing a Voronoi diagram.
%Repulsion forces try to distribute vertices that are grouped together into elements using elastic springs, 
%effectively emulating a packing process of deformable 2D elements.
}



%RepulsionPak represent an element with a set of discrete vertices.
%Similar to Lloyd's method, repulsion forces distribute vertices evenly but 
%without explicitly computes a Voronoi diagram.
%However, these vertices are collectively grouped into 2D elements, allowing
%RepulsionPak to pack and deform elements. 
%although it does not explicitly compute Voronoi diagrams.
%RepulsionPak is able to pack and deformable elements, each is
%represented as a collection of vertices that are connected using springs.
%Repulsion forces try to distribute these vertices evenly but since
%they are connected by springs, RepulsionPak can effectively
%Repulsion forces act onto vertices to distribute them evenly,
%but they are connected using elastic springs to form 2D elements.
%Repulsion forces act onto vertices that are grouped together into elements using elastic springs, 
%effectively pack and deform the elements.


%RepulsionPak can be categorized as a variant of Lloyd's method since it essentially
%aims to distribute 2D vertices, however, there are key differences: 
%(1) Vertices are connected by springs, forming a 2D element.
%(2) Repulsion forces distribute these vertices without computing a Voronoi diagram.
%\mynote{reread again}
...
This representation allows them to synthesize textures withs elements
that have bigger and more complex shapes.
...
(Fig.~\ref{fig_forces}b).  First, if two triangles 
...
displacement of these springs allows a mesh to deform in response to repulsion forces
