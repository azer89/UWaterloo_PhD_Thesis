...
with a controllable deformation model at its core, we achieve a
more even distribution of negative space, even with a small library
of element shapes. We believe that the aesthetic quality
...
convex or near convex elements as their only optimize the alignments between
element boundaries and Voronoi cells.
...
%RepulsionPak can be categorized as a variant of Lloyd's method.
%Elements are composed of vertices which are distributed by repulsion forces,
%but they are still binded together by spring.
%RepulsionPak can be categorized as a variant of Lloyd's method since
%repulsion forces aim to distribute 2D vertices, but they are connected
%together by springs.

%RepulsionPak can be categorized as a variant of Lloyd's method since it also
%aims to distribute 2D points, however, it has key differences: 
%(1) Elements are represented sampled vertices but they are binded together by elastic springs. 
%(2) Repulsion forces then distribute these vertices, but we do not compute a Voronoi diagram.
...
are also data-driven, but they permit large overlaps
...
\textbf{Physically-based methods to simulate elastic objects:}
...
%For example, 
In practice, if the container's bounding box is approximately $1000\times 1000$,
we set $k_\mathrm{rpl}=k_\mathrm{ovr}=k_\mathrm{bdr}=80$, and $k_\mathrm{tor}=1$.
We set $k_\mathrm{edg} = 10$ 
for negative-space springs, and $k_\mathrm{edg} = 40$ for edge springs and shear springs,
since weaker springs are sufficient to avoid self-intersections.
...
\mynote{Show results by artists.}
...
\mynote{Left???} The packing on the left in Figure~\ref{three_packings}
