...
\newtext{Repulsion forces} between neighboring meshes work to even out the negative
...
A packing example with primary and secondary elements \nnewtext{was} shown in Figure~\ref{fig_primary_secondary}.
}
...
\nnewtext{Elements in FLOWPAK could only bend along their spines. 
...
\nnewtext{Lloyd's} method, data-driven methods, and deformation-driven methods.
...
Forces do not act directly \nnewtext{on} elements, but their mesh vertices.
During the simulation, these mesh vertices are spread apart but \nnewtext{remain} connected using elastic springs,
effectively creating an element representation that can be packed and deformed.
Additionally, unlike previous variants of \nnewtext{Lloyd's} method, 
repulsion forces \nnewtext{naturally induce rotations in elements, helping them discover greater compatibilities with their neighbours.}. 
}
...
labyrinthine paths \nnewtext{using a geometric analogue of reaction-diffusion}. 
...
different \nnewtext{from that} of RepulsionPak (2D graphic design) and our results appear qualitatively
different.
}
...
RepulsionPak \nnewtext{uses} a similar approach, but we represent an element
...
\newtext{The containers are \nnewtext{seeded} with copies of scaled-down elements.
...
\caption[Element discretization]{
	...
	\nnewtext{An illustration of element discretization for preprocessing.}
}
\end{figure}
...
\nnewtext{Given an input element (Figure~\ref{fig_elements_image}a)}, we generate a skin, 
which is a simple closed polygon that fully encloses it \nnewtext{(Figure~\ref{fig_elements_image}b)}.
...
to obtain a simpler polygon $s'$, \nnewtext{which is the outer boundary of the mesh (Figure~\ref{fig_elements_image}c)}.
We then construct a Delaunay triangulation of $s'$ \nnewtext{(Figure~\ref{fig_elements_image}d)}.
...
created triangulation edges \nnewtext{(Figure~\ref{fig_elements_image}f)}. 
These negative space edges are used exclusively for internal
bracing.  The element's concavities can still be occupied by its neighbors.
...
This change allows the meshes \nnewtext{to be more tolerant of small displacements} and 
...
\nnewtext{For example, the Unilever logo in Figure~\ref{fig_logo_packing} has a positive space ratio of $45\%$.
...
\nnewtext{as seen in the rhinoceros's front horn in Figure~\ref{rhino_bad}.}
...
\nnewtext{We let real-world artists and users to use RepulsionPak
...
8-neighborhood around the centre.  These clones track the transformations
...
\newtext{\nnewtext{Two professional artists} have used RepulsionPak to create packings shown in Figure~\ref{paul_daichi_packings}.
The skull packing is made of butterfly and plant elements arranged inside multiple containers.
The skull's teeth \nnewtext{are} made of manually placed leaf elements. 
...
\nnewtext{Additionally, a casual user made a packing using RepulsionPak, which was then printed on
a t-shirt, as shown in Figure~\ref{adobe_packing}.}}
...
\newtext{
\nnewtext{We see several opportunities for} further improvements to RepulsionPak.
We deliberately chose a simple simulation model based on springs and forward Euler integration,
  ...
  \nnewtext{future work should} experiment with several to investigate if any offers
