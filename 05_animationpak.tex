...
\nnewtext{
The animation of RepulsionPak simulation process is enjoyable to watch, in which
elements move around to create a more even distribution and deform to create interlocking with their neighbors.
The animation inspires us to develop a method that generates dynamic arrangements in which elements are imbued with scripted animations.}
...
Excluding animated textures and animated tilings, many animated packings can be classified into one of $2\times2$ category blocks:
\begin{itemize}
\item \textbf{Static elements in a static container.} This is considered as a standard 2D packing, 
in which elements do not have animations and the container is also static.
\item \textbf{Static elements in an animated container.} A packing in 
this category has its container shape changes over time.
...
which can be interpreted as a spatial animation.}
...
\nnewtext{Additionally, we have to work toward monotonic deformation, as seen in Figure~\ref{fig_monotonicity}.
...
\nnewtext{The left element shows an example of desired deformation, 
while the right one will cause rendering artifact.
The left one has its slices monotonic and perpendicular to the time axis. 
}
}
\end{figure}
...
\nnewtext{
To render the spacetime elements properly without any rendering artifact we need to maintain two conditions.
First, all spacetime elements must be present at any time position.
Second, we need to maintain the deformation to be monotonic and the slices to be perpendicular to the time axis.
In Figure~\ref{fig_monotonicity}, we show two examples of desired and undesired deformation of a spacetime element.}  
...
\nnewtext{This constraint combined with temporal forces ensure monotonic deformation.}
