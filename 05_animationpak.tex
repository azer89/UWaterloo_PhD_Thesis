...
\nnewtext{We found only} a single animated packing, shown in Figure~\ref{fig_animationpak_lisa_packing},
\nnewtext{which is an} animated gear-like meshing of copies of Lisa Simpson's spiky head.
...
\nnewtext{When we consider adding scripted animations to a packing, 
...
\nnewtext{These are standard 2D packing algorithms that produce a single drawing of a static container packed by static elements.}
\item \textbf{Static elements in an animated container.} 
\nnewtext{These algorithms allow the container to move and deform. 
...
\nnewtext{The container is fixed, but the elements are endowed with scripted animations. 
...
placed using \nnewtext{centroidal area Voronoi diagrams (CAVDs)}, 
...
\nnewtext{Packings generated by Animosaics fall into the category of static elements in an animated container.}
...
\nnewtext{Animated packings generated by the spectral approach 
...
\nnewtext{Both techniques fall into the category of static elements in an animated container.}
...
\newtext{This work \nnewtext{falls} into the category of static elements inside an animated container.}
}

\textbf{Animated tilings:}
\newtext{We discussed metamorphosis tilings in Section~\ref{related_wrok_tiling},
consisting of \nnewtext{slowly evolving interlocked elements}.
Although the entire composition of a metamorphosis tiling is static,
\nnewtext{the evolving elements} can be interpreted as a spatial animation.}
...
\nnewtext{Inspired by DET, Kazi et al.~\cite{Kazi2014} developed an interactive user interface where
an artist can draw curves to guide the motions of animated elements.}
\nnewtext{Both techniques by Ma et al.\ and Kazi et al.\ do not belong to any category in the  
$2\times2$ grid above, as their arrangements do not have containers and elements can move freely without the need of being inside.}
...
with the goal of \nnewtext{maintaining a separation of $d_\mathrm{gap}$} between elements in the resulting packing.}
(Figure~\ref{fig_animationpak_discretization}a).  In our system
we scale the shape to fit a unit square and set \nnewtext{$d_\mathrm{gap}=0.08$}.
...
A spring connects the \nnewtext{centremost} vertex $\bm{x}$ of a slice $s$ to a target
point $p$.
...
\caption[An illustration of an acceptable and \newline an unacceptable configurations of an element]
...
\nnewtext{full extent} from $t=0$ to $t=1$, and \nnewtext{slices perpendicular} to the time axis.
}
}
\end{figure}
...
we show \nnewtext{examples} of a \nnewtext{acceptable and unacceptable} configurations of a spacetime element.
}

\newtext{
We enforce two mandatory geometric constraints and 
an optional constraint on element slices.  Each of the following
constraints is reapplied after \nnewtext{every} physical simulation step described above.
}
...
\nnewtext{we used} a pool of eight threads, corresponding to the number of logical CPU cores.}
...
\nnewtext{We call the slow motions of the lion's mane as ambient animation,
...
Like the spectral method~\cite{Dalal2006}, and unlike
  ...
  \newtext{As shown in the $2\times2$ \nnewtext{classification of algorithms} 
  ...
  \nnewtext{As a further extension we could consider} dynamically adding and removing \nnewtext{elements during the animation. 
...
It would be interesting to \nnewtext{the construction of illustrated cinemagraphs using animated element arrangements.}}
