...
the other physical forces in play (Figure~\ref{fig_simulation_process}b).
...
% Chapter~\ref{chapter_repulsionpak} Section~\ref{repulsionpak_simulation}
...
to simultaneously grow, deform, and repel each other (Figure~\ref{fig_simulation_process}).
In Sect.~\ref{sect_constraints} we introduce some new hard
...
RepulsionPak (Chapter~\ref{chapter_repulsionpak} Section~\ref{repulsionpak_simulation})~---with 
the exception of the new temporal
...
detail in Sect.~\ref{animationpak_spatial_queries}.
}

\newtext
{
Temporal forces prevent slices from drifting too far
from their original positions along the time axis positions
(Fig.~\ref{fig_t_force}), which could cause unexpected accelerations
...
In practice,
we control the relative strengths of the forces in the simulation by adjusting parameters below:
\begin{packednumbers}
\item We set the relative strength of repulsion forces by setting $k_\mathrm{rpl}$ to $10$.
\item We have five types of springs, with stiffness constants that can be 
  set independently. We set $k_\mathrm{edg}$ 
  to $0.01$ for time springs, $0.1$ for negative-space springs,
  and 10 for edge springs, shear springs, and target point springs.
\item We set $k_\mathrm{ovr} = 5$.
\item We set $k_\mathrm{bdr} = 5$.
\item We set $k_\mathrm{tor} = 0.1$.
\item We set $k_\mathrm{tmp} = 1$.
\end{packednumbers}
...
{
   \newtext{Data and statistics for the AnimationPak results}.  The table shows the
   number of elements,
...
\item As noted in Sect.~\ref{section_rendering} and Figure~\ref{fig_animationpak_blender},
