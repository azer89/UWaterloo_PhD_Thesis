...
with the goal of having $d_\mathrm{gap}$ separations between elements in the resulting packing.}
(Figure~\ref{fig_animationpak_discretization}a).  In our system
we scale the shape to fit a unit square and set \nnewtext{$d_\mathrm{gap}/2=0.04$}.
...
\nnewtext{In practice,  we shrink these elements 5--10\% of their original size.}
...
\caption[An illustration of a desired and \newline an undesired configurations of an element]
{\label{fig_monotonicity} 
\nnewtext{
The left element shows an example of a desired configuration,
while the right one will cause rendering artifact
since it does not have any of three required conditions: monotonic deformation, 
its presence from $t=0$ to $t=1$, and perpendicular slices to the time axis.
}
}
\end{figure}
...
\nnewtext{
Our system can provide robust simulations that arrange and deform spacetime extrusion.
...
we show two examples of a desired and an undesired configurations of a spacetime element.
}
...
\nnewtext{Figure~\ref{fig_animationpak_constraints}a shows an illustration of the end-to-end constraint.
This constraint only performs localized adjustments on the first and the last slices.
...
\nnewtext{Figure~\ref{fig_animationpak_constraints}b shows an illustration of the simultaneity constraint.}
...
We enable the loop constraint in all results.  \nnewtext{This chapter shows selected frames from the
...
illusion shown as a \nnewtext{RepulsionPak static packing shown in Figure~\ref{giraffe_penguin_packing}}.  
This example
...
\nnewtext{an example where a guided element is not constrained to the inside of a container}.
...
\nnewtext{This is a demonstration of the benefit of the non-uniform growth in element slices, 
allowing elements to adapt to changes in container area.}
...
\nnewtext{counter-rotating} stars.
...
Like the spectral method~\cite{Dalal2006}, and unlike
  ...
  \nnewtext{As shown in the $2\times2$ blocks (Figure~\ref{fig_animationpak_categorization}),
...
\item \newtext{\textbf{Cinemagraph}:}
\nnewtext{The lion's mane packing shows the potential of AnimationPak to generate
...
Video Textures method by Sch\"{o}dl et al.~\cite{Schodl2000} and Cliplets method by Joshi et al.~\cite{Joshi2012}.
It would be interesting to use the idea of cinemagraph into element arrangements and ornamentation.}
