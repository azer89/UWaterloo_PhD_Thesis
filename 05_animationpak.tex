...
with the addition of scripted animation for each input element.}
...
We would like to replace time springs with motion paths, each is a curve that connects slices together.
The challenge is to generate plausible motion paths that can be deformed by other forces.
We also need to detect features of an animation, which is then used as constraints for generating a motion path.
