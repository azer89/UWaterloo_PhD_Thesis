...
\newtext{
During the development of RepulsionPak, we observed that the simulation process, 
while intended only as a means to an end, was itself enjoyable to watch. 
Elements would move and deform in an active, almost organic way as they responded to physical forces. 
These animations inspired us to explore a method for generating packings in which elements are augmented with scripted animations.
}

%\nnewtext{
%The animation of RepulsionPak simulation process is enjoyable to watch, 
%showing the movements and deformation of elements.
%The animation inspires us to develop a method that generates dynamic element arrangements, in which elements are imbued with scripted animations.}
...
\newtext{We found only} a single animated packing, shown in Figure~\ref{fig_animationpak_lisa_packing},
\newtext{which is an} animated gear-like meshing of copies of Lisa Simpson's spiky head.
...
\newtext{When we consider adding scripted animations to a packing, 
we must decide separately whether to support animated elements and an animated container shape. 
Work in this area can then be categorized naturally into a $2\times2$ grid, 
as shown in Figure~\ref{fig_animationpak_categorization}.  
We discuss these categories and techniques in greater detail below.}
\begin{itemize}
\item \textbf{Static elements in a static container.} 
\newtext{These are standard 2D packing algorithms that produce a single drawing of a static container packed by static elements.}
\item \textbf{Static elements in an animated container.} 
\newtext{These algorithms allow the container to move and deform. 
The elements do not have scripted animations, 
but they might still animate passively in response to changes in the container.}
\item \textbf{Animated elements in a static container.} 
\newtext{The container is fixed, but the elements are endowed with scripted animations. 
Elements must negotiate for space within the container as they attempt to express their animations.}
\item \textbf{Animated elements in an animated container.}
In this category, both elements and the container have scripted animations.
We are not aware previous work that falls into this category.
\end{itemize}

\begin{comment}
\nnewtext{
We show a general classification of dynamic element packings in Figure~\ref{fig_animationpak_categorization}.
Many mosaicing and packings can be classified into one of $2\times2$ category blocks:
\begin{itemize}
\item \textbf{Static elements in a static container.} This is considered as a standard 2D packing, 
in which elements and the container are static.
\item \textbf{Static elements in an animated container.} A packing in 
this category has static elements arranged inside an animated container.
These static elements are still allowed to have motions of rigid transformations as 
the consequences of changes of the container shape.
\item \textbf{Animated elements in a static container.} In this category, a packing 
contains elements that have scripted animations that are arranged in a static container.
AnimationPak falls into this category.
\item \textbf{Animated elements in an animated container.}
In this category, both elements and the container have scripted animations.
We are not aware previous work that fall into this category.
\end{itemize} 
}
\end{comment}
%We provide a general classification of dynamic element packings in Figure~\ref{fig_animationpak_categorization}.
%The classification consists of four blocks, depending on two factors: 
%(1) whether elements are static or animated, and 
%(2) whether the container is static or animated.
%We define an animated element as an object that has scripted animations,
%similarly, an animated container is a shape that dynamically changes over time.
%A static element is a passive object, in an animated packing, a static element
%can still have animated movements but are dictated 
...
\newtext{A categorization of animated packings.}}
\end{figure}
...
placed using \newtext{centroidal area Voronoi diagrams (CAVDs)}. 
%and advected frame-to-frame using a choice of methods
%motivated by Gestalt grouping principles.
\nnewtext{Initially, elements are placed using \newtext{centroidal area Voronoi diagrams (CAVDs)}.
Elements are then advected frame-to-frame using a choice of methods motivated by Gestalt grouping principles,
aiming to generate group motions of elements, instead of distracting uncoordinated movements of individual elements.}
...
\newtext{Packings generated by Animosaics fall into the category of static elements in an animated container.}

%To avoid distracting uncoordinated changes of elements, they used Guided by Gestalt grouping principles, by aiming to generate group motions of elements.
%They used Guided by Gestalt grouping principles avoid distracting uncoordinated changes of elements.
%Elements are placed using \newtext{centroidal area Voronoi diagrams (CAVDs)}, 
%and advected frame-to-frame using a choice of methods
%motivated by Gestalt grouping principles.

Initially, Elements are placed using \newtext{centroidal area Voronoi diagrams (CAVDs)}.
Elements are then advected frame-to-frame using a choice of methods motivated by Gestalt grouping principles,
that aim to generate group motions of elements, instead of distracting uncoordinated movements of individual elements.
...
\newtext{Animated packings generated by the spectral approach 
fall into the category of animated elements in a static container.}
...
\newtext{Both techniques fall into the category of static elements in an animated container.}
...
\newtext
{
\textbf{Animated 3D Packings:} Theobalt et al.~\cite{Theobalt2007}
developed a data-driven method to generate animated 3D collages that resemble \nnewtext{Arcimboldo paintings} by arranging 
static 3D elements into an animated 3D container.
They \newtext{partitioned} the input animated container by identifying
rigid parts, \nnewtext{each of which is replaced} by a matching element taken from a library.
...
\newtext{with some degree of deformation to improve the fit with the container.}
\newtext{This work \newtext{falls} into the category of static elements inside an animated container.}
}

\textbf{Animated tilings:}
\newtext{We discussed metamorphosis tilings in Section~\ref{related_wrok_tiling},
consisting of \newtext{slowly evolving interlocked elements}.
Although the entire composition of a metamorphosis tiling is static,
\newtext{the evolving elements} can be interpreted as a spatial animation.}
More recently, Kaplan~\cite{Kaplan2019} explored animations of simple tilings of
...
\newtext{such as a school of fish} 
\newtext{or particles.
...
\newtext{Inspired by DET, Kazi et al.~\cite{Kazi2014} developed an interactive user interface where
an artist can draw curves to guide the motions of animated elements.}
\newtext{Both techniques by Ma et al.\ and Kazi et al.\ do not belong to any category in the  
$2\times2$ grid above, as their arrangements do not have containers and elements can move freely.}
...
\nnewtext{The input to AnimationPak is similar to ones described in Section~\ref{problem_formulation},
with the addition of scripted animation for each element.}
\deadtext{The input to AnimationPak is a library of animated elements and a
...
vector paths.  \newtext{As with RepulsionPak}, we construct a discrete
...
we choose a number of slices \nnewtext{$n_\mathrm{s}$} and place slices \nnewtext{$\{s_1,\ldots,s_{n_\mathrm{s}}\}$},
with slice $s_i$ being placed at time \nnewtext{$(i-1)/(n_\mathrm{s}-1)$}.
Higher temporal resolution will
produce a smoother final animation at the expense of more computation.
In our examples, we set $n_\mathrm{s}=100$. 
Figure~\ref{fig_animationpak_discretization}d shows a set of time slices, with
$n_\mathrm{s}=5$ for visualization purposes.
...
transformations.
Figure~\ref{fig_animationpak_discretization}e shows time edges for $n_\mathrm{s}=5$.
...
(a) A 2D element shape offset by \newtext{$d_\mathrm{gap} / 2$}.
...
(e) The vertices on the boundaries of the slices are joined by 
  ...
  In practice we use a larger number of slices \nnewtext{than} in (d) and (e).
...
%only, to 5--10\% of its original size. 
%\nnewtext{As seen in Figure~\ref{fig_animationpak_simulation_process}a,} these shrunken elements are 
...
\newtext{As seen in Figure~\ref{fig_animationpak_simulation_process}a,} these shrunken elements are
...
\newtext{In practice,  we shrink these elements \newtext{to} 5--10\% of their original size.}
...
A spring connects the \newtext{centremost} vertex $\bm{x}$ of a slice $s$ to a target
point $p$.
...
\newtext{a given vertex $\bm{x}$ accumulates repulsion forces from points at different time positions on neighbouring elements.}
...
allowing more tolerance to small displacements while avoiding severe deformation.}
...
\nnewtext{Each spring type} has a different relative strength.
}
%\nnewtext
%{The edge forces are calculated using a quadratic version of Hooke's law, 
...
neighbouring spacetime element.
...
from their original positions along the time axis (Figure~\ref{fig_animationpak_t_force}).
Drifting slices can cause two problems.
First, they could cause unexpected accelerations and decelerations in the final animation. 
Second, the deformation of the spacetime element would not be monotonic,
and this would cause rendering artifacts
as illustrated in Figure~\ref{fig_monotonicity}.}
...
\begin{packeddescriptions}
  ...
  \item[$\bm{u}^t = (0,0,1)$.]
\end{packeddescriptions}
%\nnewtext{Additionally, temporal forces can
%help us to work toward monotonic deformation, as illustrated in Figure~\ref{fig_monotonicity}.
%If the deformation is monotonic, we can prevent rendering artifacts.
%This can be achieved using the combination of temporal forces and slice constraints that
%we will discuss in Section~\ref{animationpak_slice_constraints}.}
...
$s'_i$ are drawn back towards time $t$.
}
\end{figure}
...
%fig_monotonicity
%animationpak_slice_constraints
...
\caption[An illustration of an acceptable and \newline unacceptable configurations of an element]
{\label{fig_monotonicity} 
\newtext{
The left element shows an example of a desired configuration,
while the right one will cause rendering \newtext{artifacts}
since it does not have any of three required conditions: monotonic deformation, 
\newtext{full extent} from $t=0$ to $t=1$, and \newtext{slices perpendicular} to the time axis.
}
}
\end{figure}
...
\newtext{
Our system can provide robust simulations that arrange and deform \newtext{spacetime extrusions}.
However, to eliminate potential problems during the rendering,
we have to enforce two additional conditions.
First, all spacetime elements must be present at any time position.
Second, element slices have to be perpendicular to the time axis.
In Figure~\ref{fig_monotonicity}, 
we show \newtext{examples} of \newtext{acceptable and unacceptable} configurations of a spacetime element.
}

\newtext{
We enforce two mandatory geometric constraints and 
an optional constraint on element slices.  Each of the following
constraints is reapplied after \newtext{every} physical simulation step described above.
}
...
\newtext{Figure~\ref{fig_animationpak_constraints}a shows an illustration of the end-to-end constraint.
This constraint only performs localized adjustments on the first and last slices.
Over time, these adjustments will spread throughout the elements as 
edge forces strive for equilibrium.}
...
\newtext{Figure~\ref{fig_animationpak_constraints}b shows an illustration of the simultaneity constraint.}
...
\newtext{The processor is a quad core CPU with hyperthreading; 
\newtext{we used} a pool of eight threads, corresponding to the number of logical CPU cores.}
...
results; see supplemental videos for full animations.}
...
illusion shown as a \newtext{RepulsionPak static packing shown in Figure~\ref{giraffe_penguin_packing}}.  
This example uses torsional forces to control slice orientations.
...
\newtext{an example where a guided element is not constrained to the inside of a container}.
The traveling bird moves from left to right, passing through a packing of birds.
As some slices of the traveling bird \newtext{take} up container space,
the other birds have to stop some of their slice \newtext{growth} prematurely.
\newtext{This is a demonstration of the benefit of the non-uniform growth in element slices, 
allowing elements to adapt to changes in \newtext{available} container area.}
...
Figure~\ref{fig_animationpak_lion}a is a static packing of a \newtext{lion's mane} created by an artist \newtext{and
used as an example of flow-based packings (Chapter~\ref{chapter_introduction}) Figure~\ref{fig_dog_flow}}.  In Figure~\ref{fig_animationpak_lion}b,
...
\newtext{We call the slow motions of the lion's mane as ambient animation,
a topic that we would like to pursue as future work.}
%\nnewtext{This is a demonstration of a packing with ambient animation,
%consisting of static elements combined with slowly animated elements.}


%\nnewtext{This packing demonstrates ambient animation,
%which are slower than other results.}

%\nnewtext{This is a demonstration of a packing with ambient animation,
%consisting of static elements combined with slowly animated elements.}
%We consider this packing as a demonstration of ambient animation,
%where slow motions of the lion's mane is combined with static elements
...
with different time spring stiffness.  In (a), the time springs
...
\newtext{The visualization in (c) shows the trajectories of two identical elements in both cases, 
which makes it clear that weak time springs cause more frantic movements.}
...
\item \nnewtext{\textbf{Motion Paths:}
Element motions in resulting packings can look chaotic due to the interaction between time springs and repulsion forces.
It may be possible to replace time springs with motion paths, each is a curve that connects slices together.
The challenge is to generate plausible motion paths that can be deformed by other forces.
We also need to detect motion extrema of an animation, which are then used as constraints when generating a motion path.
We would like to investigate motion path editing techniques such as work by Gleicher~\cite{Gleicher2001} and Lockwood and Singh~\cite{Lockwood2011}. 
}
...
Like the spectral method~\cite{Dalal2006}, and unlike
  ...
  \newtext{As shown in the $2\times2$ \newtext{classification of algorithms} 
  (Figure~\ref{fig_animationpak_categorization}),
  we are not aware any previous method that can pack animated elements inside an animated container.  
  ...
  \newtext{As a further extension we could consider} dynamically adding and removing \newtext{elements during the animation. 
    This would certainly complicate the process of giving every element an initial placement that is fully 
    inside the container and disjoint from other elements.}}

\item \newtext{\textbf{Cinemagraphs}:}
\newtext{The lion's mane packing shows the potential of AnimationPak to generate
a cinemagraph, a composition with repeating slow ambient animations combined with static elements.
There are many image processing techniques to generate cinemagraphs, for example, 
Video Textures~\cite{Schodl2000} and Cliplets~\cite{Joshi2012}.
It would be interesting to \newtext{the construction of illustrated cinemagraphs using animated element arrangements.}}
...
\caption[A comparison between CAVD, The spectral approach, \newline and AnimationPak]
