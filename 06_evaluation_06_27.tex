...
%In the end we discarded the idea since we find they have underlying issues.
%However, by explaining them, it would become clear on reasons why we decide to use the proposed quantitative metrics.
%In this section, we discuss distance transforms applied on the negative space as they give us a hint of the evenness.
%We also considered an idea of evaluating distance transforms only on two structures: Voronoi cell boundaries and medial axes.
%Although we discarded the idea since we find these structures have underlying issues, 
%we think discussing them would explain readers' questions that might arise
%regarding our decision to use the proposed quantitative metrics. 
}
...
%In theory, we can evaluate the distance transform only on pixels 
%that belong to the medial axis to determine the evenness of negative space.
...
spherical contact probability functions, and histograms of distance transform.
}
