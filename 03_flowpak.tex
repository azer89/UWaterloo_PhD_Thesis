...
\mynote{points or vector should be bold!}
...
\caption[An example of flow visual style]{
  \label{dog_flow}  
  \mynote{Maybe show other examples}
...
\mynote{This first paragraph looks good}
\newtext
{
...
A hand-drawn dog example in Figure~\ref{dog_flow} shows many elements that appear to flow
...
In studying designs like those in Figure~\ref{some_examples}, we 
...
\item \textbf{Flow.} In local parts of a composition, the elements
  ...
  All of the examples in Figure~\ref{fig:some_examples} exhibit 
...
\item \textbf{Fixed elements.} Compositions use a small number of fixed
  ...
  every example in Figure~\ref{fig:some_examples} has eyes drawn
  ...
  %as in the empty region around the fish's eye. %%% REZA BACKUP
...
on the generation of packings or mosaics.  See Section~\ref{related_work}
for specific examples.  However, this past work is not appropriate
for creating designs like those of Figure~\ref{fig:some_examples}.
Most techniques pack elements via rigid transformations, leading to
high uniformity but insufficient variety.  We believe that there
is value in articulating systematic modes of geometric deformation
that can generate plausible families of related decorative elements
...
as the lion and unicorn shown in Figure~\ref{lion_unicorn}, aim to obey the
...
Asente's brush app (I forget the name)
...
\WHILE {$P$ is not empty}
  ...
  \IF {$s$ is longer than $s\_min$}
    \STATE Add $s$ to $S$.
    \STATE Create seed points that are $s\_gap$ away from $s$ and
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{figures/flowpak/result_02.pdf} %RHINO
...
\bigskip
\includegraphics[width=0.5\textwidth]{figures/flowpak/bear_leaves.pdf} %BEAR LEAVES
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{figures/flowpak/cat.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{figures/flowpak/dog_flower.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{figures/flowpak/bear_offset_space.pdf}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{figures/flowpak/bird_square.pdf}
...
%\item \textcolor{red}{If we compare our results with examples in Figure\ref{some_examples}, one obvious future work is eliminating the negative space even more.}
...
\item \mynote{delet this or move to chapter} As with many research projects in non-photorealistic rendering, this
