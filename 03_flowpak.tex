...
Related methods in fabrication \newtext{have} sought to cover surfaces with
...
\newtext{The user provides \nnewtext{several inputs, similar to those} described in Section~\ref{problem_formulation} with a few additional details:}
\begin{enumerate}
\item \newtext{An input element has a spine that will control its deformation.}
\item \newtext{\nnewtext{Each container includes} a set of direction guides that influence the placement algorithm, defining the flow of the results.
...
\item \newtext{Fixed elements are also included.}
\end{enumerate}

\nnewtext{
The goal of FLOWPAK is to fill each target container with elements.
Figure~\ref{fig_flowpak_pipeline} gives an overall view of our system. 
The drawing in Figure~\ref{fig_flowpak_pipeline}(1) shows containers, directional guides, and a fixed element.
The drawing in Figure~\ref{fig_flowpak_pipeline}(2) shows elements and their spines.
The numbers in the following steps correspond to parts of the figure.}
...
\begin{enumerate}
  ...
  \item Use the direction guides to fill each target container with a vector field \newtext{and then} trace streamlines 
  (Section~\ref{flowpak_creating_vector_fields_and_tracing_streamlines}).
  \item Divide the target containers into Voronoi cells around \nnewtext{the streamlines. Each cell is referred to} as a \textit{blob}. (Section~\ref{flowpak_subregion_blobs}).
...
\newtext{The refinement process aims to reduce the amount of negative space and make it
