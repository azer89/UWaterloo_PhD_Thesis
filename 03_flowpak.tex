...
We aim to create packings that obey design principles articulated in Chapter~\ref{chapter_introduction},
...
Xu and Mould~\cite{Xu2009} \nnewtext{generated decorative curves by simulating the trajectories of charged particles in a magnetic field.}
...
Elements must be \nnewtext{linked together} to produce a connected result
...
%input\_size
We define \nnewtext{$l_\mathrm{canvas}$ to be the canvas size, which is the maximum of the combined width or height of all the
...
We define two classes of elements: a \nnewtext{\textit{two-sided element}} extends across
both sides of its
spine, and a \nnewtext{\textit{one-sided element}} lies entirely on one side of its spine.
Figure~\ref{ornamental_shapes_fig} shows examples of \nnewtext{two-sided and one-sided elements}.  
If the input to our algorithm includes direction guides that coincide with target container boundaries,
the placement method will align \nnewtext{one-sided elements} along these boundaries.  
\nnewtext{The alignment of one-sided elements will visually reinforce
...
left and right of the spine. \nnewtext{The LR function is the set $\{L, R\}$ where $L=\{\ell_1,\ldots,\ell_{n_\mathrm{f}}\}$
...
Ornamental elements and their LR functions. \nnewtext{Two-sided elements} have non-empty
left and right sides, while \nnewtext{one-sided elements} have only one non-empty side. 
...
\item[\nnewtext{$s_\mathrm{gap}$}] is the desired space between streamlines;
\item[\nnewtext{$s_\mathrm{max}$}] is the maximum desired streamline length; and
\item[\nnewtext{$s_\mathrm{min}$}] is the minimum desired streamline length.   
...
compute \newtext{$s_\mathrm{stop} = 0.8\,s_\mathrm{stop}$}.
}
...
We use a sampling distance of \nnewtext{$0.005\,l_\mathrm{canvas}$}.
...
$s$, again using \nnewtext{$0.005\,l_\mathrm{canvas}$}, and at each point generate two more
...
\nnewtext{will naturally place} \nnewtext{one-sided elements} along streamlines that follow container
...
\nnewtext
{
We have created an initial arrangement consists 
of flow-guided elements in each distinct container.
We now merge all the containers and all elements within them.
We perform this part of the algorithm in an iterative manner,
allowing elements to interact with each other.
}
...
\nnewtext{$g_\mathrm{limit}=0.005\,l_\mathrm{canvas}$ (Recall that $l_\mathrm{canvas}$} is the bounding box
...
For a node, we check its $150$ nearest neighbors, considering only neighbors where the angle between the line into the current node and the line to the neighbor form an angle greater than $90^\circ$, thereby preventing
...
\bm{v} 		   & \nnewtext{a sampled vector of the vector field at $\bm{p_a}$}; \\
...
\nnewtext{$g_\mathrm{limit}=0.005\,l_\mathrm{canvas}$.}
...
with varying angles \nnewtext{$r_\mathrm{angle} = \pm \{1^{\circ}, 2^{\circ}, 3^{\circ}, ..., 10^{\circ}\}$}
...
We \nnewtext{compute only} the left part and reflect the result.
...
{A packing of the same container as in Figure~\ref{result_bear_leaves}.
  ...
  \nnewtext{The bottom row is the visualization of primary elements in black and
